...
class ProjectiveGridGenerator(nn.Module):
    def __init__(self, size_ipm, M, no_cuda):
        ...
        if not no_cuda:
            self.base_grid = self.base_grid.cpu()
            # self.im_h = self.im_h.cpu()
            # self.im_w = self.im_w.cpu()
...
class Net(nn.Module):
    def __init__(self, args, input_dim=1, debug=False):
        ...
        if not self.no_cuda:
            self.M_inv = self.M_inv.cpu()
            self.S_im = self.S_im.cpu()
            self.S_im_inv = self.S_im_inv.cpu()
            self.S_im_inv_batch = self.S_im_inv_batch.cpu()
            self.H_c = self.H_c.cpu()
            self.K = self.K.cpu()
            self.H_g2cam = self.H_g2cam.cpu()
            self.H_ipmnorm2g = self.H_ipmnorm2g.cpu()
            self.cam_height_default = self.cam_height_default.cpu()
            self.cam_pitch_default = self.cam_pitch_default.cpu()
