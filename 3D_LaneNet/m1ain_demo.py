...
from dataloader.L1oad_Data_ext import *
from networks import G1Net3D_ext, e1rfnet
...
if __name__ == '__main__':
    ...
    # Define network
    model_seg = e1rfnet.ERFNet(2)  # 2-class model
    model_geo = G1Net3D_ext.Net(args)
    ...
    if not args.no_cuda:
        ...
        model_geo = model_geo.cpu()

    # load segmentation model
    #checkpoint = torch.load(pretrained_feat_model)
    checkpoint = torch.load(pretrained_feat_model, map_location=torch.device('cpu'))
    ...
    if os.path.isfile(trained_geo_model):
        ...
        checkpoint = torch.load(trained_geo_model, map_location=torch.device('cpu'))
        model_geo.load_state_dict(checkpoint['state_dict'], strict=False)
        model_geo.eval()
    else:
