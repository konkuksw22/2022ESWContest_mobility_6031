...
* ~9월 5일
> lane detect감지 방법 연구 중, RANSAC Sequential 알고리즘을 통해 도로의 평행 직선을 추출하는 알고리즘 연구.

* ~9월 10일
> detect한 차선을 통해 multi roi를 생성시킴. 그 후 해당 lane의 타입을 color mask를 통해 알아내고, 해당 차선에서 우회전 의도하는 것인지 판단.   
...
3) multi ROI 생성 및 해당 lane의 type 판별   
RANSAC sequential을 통해 평행 차선을 인식하고자 하였고, color masking을 통해 해당 차선의 type을 판별하도록 하였음.   
차선이 차량의 중앙에서 많이 벗어날 시, 해당 차량이 차선변경하였음을 지각하게 해서 해당 차량이 교차로에서(횡단보도 인식과 더불어) 우회전 시도를 한다는 것을 판별하게 한다.   
...
최종) 차선인식 -> 현재 차량의 차선 위치 인식 -> 교차로 인식(횡단보도) -> 차량의 중앙이 인식한 차선에서 벗어난 경우 우회전 의도 파악 -> 사물 인식 (추후, 보행자가 횡단보도에서 벗어나고 있는지 멀어지고 있는지 파악)
