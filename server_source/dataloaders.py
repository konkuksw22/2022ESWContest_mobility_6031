...
mtx=np.array([[1.20229938e+03, 0.00000000e+00, 6.35201297e+02],[0.00000000e+00, 1.20810475e+03, 3.68400749e+02],[0.00000000e+00, 0.00000000e+00, 1.00000000e+00]])
dist=np.array([[-0.72730807, 0.33691676, 0.01186183, 0.00749736, 0.0398664]])
...
class LoadImages:
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            ...
            #crop = im0[crop_val:height, 0:width]
            ...
            # intensity = np.mean(im0)
            # print("Intensity :", intensity)
            # if intensity < 110:
            #     im0[:,:,0] = cv2.add(im0[:,:,0], 50)
            ...
            # crop = cv2.cvtColor(crop, cv2.COLOR_BGR2YUV)
            # clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8,8))
            # crop[:,:,0] = clahe.apply(crop[:,:,0])
            # crop = cv2.cvtColor(crop, cv2.COLOR_YUV2BGR)
            # im0[450:height, 0:width] = crop

            # Hist Equalize ROI Region-----------------------------------------------------------------------
            # crop = cv2.cvtColor(crop, cv2.COLOR_BGR2YUV)
            # crop[:,:,0] = cv2.equalizeHist(crop[:,:,0])
            # crop = cv2.cvtColor(crop, cv2.COLOR_YUV2BGR)
            #im0[crop_val:height, 0:width] = crop
