...
lane_width=0
middle_width_lane=np.hstack((0., 0.))
margin_search_true=False
...
# 방향 지시등 그리기
def draw_arrow(img):
    return cv2.arrowedLine(img, (1080, 80),(1235, 80), red, 8, 8, 0, 0.4)
...
def region_of_interest(img, vertices):
        ...
        if len(img.shape) > 2:
            ...
            remove_middle_color=(0, )*channel_count
        else:
            ignore_mask_color = 255
            remove_middle_color=0
...
def hough_lines(img, rho, theta, threshold, min_line_len, max_line_gap, Minv):
...
def remove_middle_lane(image):
    ...
    vertices=np.int32([
        [left_line.bestx+lane_width/4, 720], 
        [left_line.best_fit[2]+lane_width/4, 0], 
        [left_line.best_fit[2]+lane_width*3/4, 0], 
        [right_line.bestx+lane_width/4, 720]]) #이미지 크기에 따라 달라짐
    cv2.fillPoly(mask, vertices, (255,255,255))
    cv2.imshow("is real", mask)
...
def process_image(image):
    global first_frame, prev_line_img
    ...
    b_bin = lab_b_channel(warped, thresh = (185, 245))
    color_mask[(mask_white==1)|(b_bin>=1)]=255
    cv2.imshow("mask_show", color_mask)
    ...
    vertices=[np.array([[435, 720],[400, 0],[850, 0],[850, 720]])] #Bird eye view의 ROI
    roi_image = region_of_interest(color_mask, vertices)
    cv2.imshow("ROI", roi_image)

    line_image, dst = hough_lines(roi_image, rho, theta, thresh, min_line_len, max_line_gap, Minv)
    hough_result=weighted_img(line_image, image, α=0.8, β=1., λ=0.)
    cv2.imshow("perspective+hough", hough_result)

    output_img=find_lanes(dst)

    cv2.imshow("pi", output_img)

    result = weighted_img(output_img, warped, α=0.8, β=1., λ=0.)
    cv2.imshow("test", result)

    #lane_img=draw_lane(image, dst, Minv)
    newwarp = cv2.warpPerspective(output_img, Minv, (image.shape[1], image.shape[0])) 
    lane_img=cv2.addWeighted(image, 1, newwarp, 0.6, 0)
    cv2.imshow("result_assemble", lane_img)

    clip1.write(lane_img)

    #result_assem=assemble_img(warped, dst, output_img, lane_img)
...
def pers_transform(img, nx=9, ny=6):
    ...
    #     vertices1 = np.array([[260, 485],[550, 345],[645, 345],[900, 485],])
...
"""
validation 기법이랑, 가운데 부분 삭제하는거 찾기
"""
...
def mask_image_2(image, vertices):
    masked_image = np.copy(image)
    mask = np.zeros_like(masked_image)
    cv2.fillPoly(mask, vertices, (255,255,255))
    masked_edges = cv2.bitwise_and(masked_image, mask)
...
def process_img(img):
    ...
    # vertices=[np.array([[435, 720],[400, 0],[850, 0],[850, 720]])] #Bird eye view의 ROI
    # warped = mask_image_2(warped, vertices)
    
    # Colour thresholding in S channel
    s_bin = hls_thresh(warped)
    cv2.imshow("combined", s_bin)
...
frames = 0
first_frame = 1
left_fit_prev=np.array([0., 0., 0.], np.float64)
right_fit_prev=np.array([0., 0., 0.], np.float64)
...
"""--------------------------Video test--------------------------------------"""
thread_key=threading.Thread(target=detect_key)
...
while (cap.isOpened()):
    ret, frame = cap.read()
    if frames==0:
        prev_line_img=np.zeros_like(frame)
    if ret:
        ...
        if frames %3 == 0: # Frame 높히기; 눈속임
            ...
            #방향 지시등 표시해주기
            if arrow_state==True:
                lane_detection=draw_arrow(lane_detection)
