...
middle_point=(630, 540)  #시야로 확인한 중간지점
#middle_point=(0,0)
h1_y=720 #h1 직선의 y좌표
h2_y =555 #h2 직선의 y좌표
...
r_r_Roi=np.array([[768+lane_width+100-scan_hwidth, img_height], [768+lane_width-scan_hwidth+100, 0], [768+lane_width+scan_hwidth+100, 0], [768+lane_width+scan_hwidth+100, img_height]])
...
def draw_lines(img, lines, ori_img):
    ...
    if abs(l_x1-r_x1)>200 or abs(l_x1-r_x1)<50: #맨 위쪽 차폭 제한
        return None
    ...
    if lines is not None:
        for line in lines:
            for x1,y1,x2,y2 in line:
                ...
                if (abs(h1_line_x1) <(img_width/2)) and (abs(h2_line_x2) <(img_width/2)) and abs(slope)<5 and abs(h1_line_x1-h2_line_x2)<(img_width/2): #slope조건 빠지면 양옆 차선(4개) 모두 인식함
...
def lane_position(image, gap = 20, length=20, thickness=2, color = red, bcolor = white, center=img_width/2): # length는 선의 위쪽 방향으로의 길이
    ...
    if lane_change_detected==0:
        ...
        if (l_center[0]<center-15) and r_center[0]>center+15:
            lane_change_detected=0
...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            cv2.rectangle(im0, (0, 720 // 10 * 5), (1280, 720 // 10 * 8), (0, 255, 0),
                          3)  # Hill
            cv2.rectangle(im0, (0, 0), (1280 // 5, 720), (0, 0, 255),
                          3)  # Left A pillar
            cv2.rectangle(im0, (1280 // 5 * 4, 0), (1280, 720), (255, 0, 0),
                          3)  # Right A pillar
            cv2.rectangle(im0, (0,0), (330, 130), dark, -1)
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if cswalk_detected==1:
                        ...
                        cv2.putText(frame, "crossWalk!", (400, 500), font, 0.6, red, 2)
            ...
            message = str(L_car) + str(L_person) + str(L_bicycle) + str(L_motorcycle) + str(H_car) + str(H_person) + str(H_bicycle) + str(
                L_motorcycle) + str(right_stat) + str(R_person)
            print(message)
...
"""python main_yolov5.py --source test_videos\drive_00.mp4 --weights weights/yolov5n.pt --img 640 --view-img"""